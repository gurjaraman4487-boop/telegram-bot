...
def generate_checksum(params_str, merchant_key):
    salt = "1234567890" # Custom simple salt
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_photo(
        photo=START_IMAGE,
        caption="<b>🔥 𝐏𝐑𝐄𝐌𝐈𝐔𝐌 𝐕𝐈𝐃𝐄𝐎 𝐂𝐎𝐋𝐋𝐄𝐂𝐓𝐈𝐎𝐍 🔥</b>\n\n<b>⚡ 𝐈𝐍𝐒𝐓𝐀𝐍𝐓 𝐀𝐂𝐂𝐄𝐒𝐒</b>",
        parse_mode="HTML",
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "premium":
        keyboard = [
            ...
            [InlineKeyboardButton("👑 𝐕𝐈𝐏 𝐀𝐋𝐋 - ₹499", callback_data="p4")],
            [InlineKeyboardButton("⬅️ 𝐁𝐀𝐂𝐊", callback_data="home")]
        ]
        await query.message.edit_media(media=InputMediaPhoto(media=PREMIUM_IMAGE, caption="<b>💎 𝐒𝐄𝐋𝐄𝐂𝐓 𝐘𝐎𝐔Ｒ 𝐏𝐋𝐀𝐍 💎</b>", parse_mode="HTML"), reply_markup=InlineKeyboardMarkup(keyboard))
    
    elif data == "home":
        await query.message.edit_media(media=InputMediaPhoto(media=START_IMAGE, caption="<b>🔥 𝐏𝐑𝐄𝐌𝐈𝐔𝐌 𝐕𝐈𝐃𝐄𝐎 𝐂𝐎𝐋𝐋𝐄𝐂𝐓𝐈𝐎𝐍 🔥</b>", parse_mode="HTML"), reply_markup=home_buttons())
        ...
        if not link:
            # Fallback agar staging issue ho
        ...
        await query.message.edit_media(media=InputMediaPhoto(media=PREMIUM_IMAGE, caption=f"<b>💸 𝐏𝐋𝐀𝐍: ₹{amount}</b>\n\nOrder ID: <code>{order_id}</code>", parse_mode="HTML"), reply_markup=InlineKeyboardMarkup(keyboard))

    elif data.startswith("check_"):
        order_id = data.replace("check_", "")
        status = check_paytm_status(order_id)
        if status == "TXN_SUCCESS" or status == "PENDING": 
            # Test mode me success ya pending dono par pass kar rahe hain testing ke liye
...
print("✅ BOT STARTED WITHOUT EXTERNAL CHECKSUM LIBRARY...")
app.run_polling()
